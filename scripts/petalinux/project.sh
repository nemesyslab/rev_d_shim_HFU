...
echo "[PTLNX PROJ SCRIPT] Checking PetaLinux configuration files for ${PBV}"
...
echo "[PTLNX PROJ SCRIPT] Deleting any existing PetaLinux project directory"
...
if [ "$PETALINUX_YEAR" -lt 2024 ]; then
  echo "[PTLNX PROJ SCRIPT] Using legacy PetaLinux project creation command for year ${PETALINUX_YEAR}"
  petalinux-create -t project --template zynq --name petalinux --force
else
  echo "[PTLNX PROJ SCRIPT] Using PetaLinux project creation python arguments (for year ${PETALINUX_YEAR})"
  petalinux-create project --template zynq --name petalinux --force
fi
cd petalinux

# Initialize the project with the hardware description
echo "[PTLNX PROJ SCRIPT] Initializing default PetaLinux system configuration"
...
if [ -f "$PETALINUX_CONF_PATH" ]; then
  ...
  if [ "$PETALINUX_VERSION" != "$CONF_XILINX_VER_MAIN" ]; then
    echo "[PTLNX PROJ SCRIPT] ERROR: PETALINUX_VERSION (${PETALINUX_VERSION}) does not match XILINX_VER_MAIN (${CONF_XILINX_VER_MAIN}) in petalinux-version.conf. This likely means you have the wrong or missing PETALINUX_VERSION environment variable set."
    exit 1
  fi
else
  echo "[PTLNX PROJ SCRIPT] ERROR: petalinux-version.conf not found at $PETALINUX_CONF_PATH"
  exit 1
fi

# Patch the project configuration
echo "[PTLNX PROJ SCRIPT] Patching and configuring PetaLinux project"
...
echo "[PTLNX PROJ SCRIPT] Initializing default PetaLinux root filesystem configuration"
petalinux-config -c rootfs --silentconfig
echo "[PTLNX PROJ SCRIPT] Patching and configuring PetaLinux root filesystem"
