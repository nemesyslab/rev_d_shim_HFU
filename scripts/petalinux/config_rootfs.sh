...
echo "[PTLNX ROOTFS CFG SCRIPT] Checking XSA file and PetaLinux system config file for ${PBV}"
...
if [ -f "projects/${PRJ}/cfg/${BRD}/${VER}/petalinux/${PETALINUX_VERSION}/rootfs_config.patch" ] && [ $UPDATE -ne 1 ]; then
    echo "[PTLNX ROOTFS CFG SCRIPT] ERROR:"
...
if [[ "$PETALINUX_VERSION" =~ ^([0-9]{4}) ]]; then
    ...
    echo "[PTLNX ROOTFS CFG SCRIPT] ERROR: Invalid PetaLinux version format (${PETALINUX_VERSION}). Expected format: YYYY.X"
    exit 1
fi
...
if [ "$PETALINUX_YEAR" -lt 2024 ]; then
    echo "[PTLNX ROOTFS CFG SCRIPT] Using legacy PetaLinux project creation command for year ${PETALINUX_YEAR}"
    petalinux-create -t project --template zynq --name petalinux --force
else
    echo "[PTLNX ROOTFS CFG SCRIPT] Using PetaLinux project creation python arguments (for year ${PETALINUX_YEAR})"
...
echo "[PTLNX ROOTFS CFG SCRIPT] Initializing default PetaLinux system configuration"
...
if [ -f "$PETALINUX_CONF_PATH" ]; then
    ...
    if [ "$PETALINUX_VERSION" != "$CONF_XILINX_VER_MAIN" ]; then
        echo "[PTLNX ROOTFS CFG SCRIPT] ERROR: PETALINUX_VERSION (${PETALINUX_VERSION}) does not match XILINX_VER_MAIN (${CONF_XILINX_VER_MAIN}) in petalinux-version.conf. This likely means you have the wrong or missing PETALINUX_VERSION environment variable set"
        exit 1
    fi
else
    echo "[PTLNX ROOTFS CFG SCRIPT] ERROR: petalinux-version.conf not found at $PETALINUX_CONF_PATH"
    exit 1
fi

# Patch the project configuration
echo "[PTLNX ROOTFS CFG SCRIPT] Patching and configuring PetaLinux project"
...
echo "[PTLNX ROOTFS CFG SCRIPT] Initializing default PetaLinux root filesystem configuration"
petalinux-config -c rootfs --silentconfig

# Copy the default root filesystem configuration
echo "[PTLNX ROOTFS CFG SCRIPT] Saving default PetaLinux root filesystem configuration"
cp project-spec/configs/rootfs_config project-spec/configs/rootfs_config.default

# If updating, apply the existing patch
if [ $UPDATE -eq 1 ]; then
    echo "[PTLNX ROOTFS CFG SCRIPT] Applying existing PetaLinux root filesystem configuration patch"
...
echo "[PTLNX ROOTFS CFG SCRIPT] Manually configuring PetaLinux root filesystem"
petalinux-config -c rootfs

# Create a patch for the root filesystem configuration
echo "[PTLNX ROOTFS CFG SCRIPT] Creating PetaLinux root filesystem configuration patch"
...
echo "[PTLNX ROOTFS CFG SCRIPT] Restoring default PetaLinux root filesystem configuration for template project"
